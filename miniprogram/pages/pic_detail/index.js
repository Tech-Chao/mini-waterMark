...
Page({
  ...
  onLoad: function (options) {
    if (app.globalData) {
      const { pics, title } = app.globalData['K_VIDEO_INFO'];
      console.log(app.globalData['K_VIDEO_INFO']);
      this.setData({ pics, pics_title: title });
    }
    // 在页面onLoad回调事件中创建插屏广告实例
    if (wx.createInterstitialAd) {
      interstitialAd = wx.createInterstitialAd({ adUnitId: 'adunit-2866aa30ac25cf24' });
  ...
  saveAllPics: function() {
    ...
    this.data.pics.forEach(url => {
      ...
      wx.getImageInfo({
        src: url,
        success: function(res) { console.log(res.path) },
      })
    });
  },
