...
Page({
  ...
  initRewardedVideoAd: function (adUnitId) {
    ...
    videoAd.onClose((res) => {
      const { isEnded } = res;
      if (isEnded) {
        wx.cloud.callFunction({ name: 'add_ad_view_record', data: { app_name: app.app_name } }).catch(err => { console.error(err) });
  ...
  onLoad: function (options) {
    ...
    // 检查是否需要观看激励视频才能执行下载
    wx.cloud.callFunction({ name: 'check_free_ad_status', data: { app_name: app.app_name } })
    .then(res => {
      console.log(res);
    ...
    .then(show_ad => {
      ...
      if (show_ad === 1) videoAd = this.initRewardedVideoAd('adunit-1651720533028827');
      wx.hideLoading();
    })
    .catch(err => {
      console.error(err);
    ...
    // 获取配置信息
    wx.cloud.callFunction({ name: 'fetch_config' }).then(res => {
