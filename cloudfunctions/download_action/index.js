...
exports.main = async (event, context) => {
  ...
  // 存储 fileID 和时间，定时清理
  const record = { ...result, is_deleted: 0, created_time: moment().format('YYYY-MM-DD HH:mm:ss') };
  db.collection('p_fileid').add({ data: record }).then(res => console.log('add file id', res)).catch(err => console.error(err));
  return result;
}
